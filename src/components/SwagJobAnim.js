...
export default function SwagModel({ ...props }) {
  const group = useRef()
  const rotateB = useRef()
  const { nodes, materials, animations } = useGLTF('/swagJobAnim.glb')
  const { actions } = useAnimations(animations, group)


  // useFrame(( { clock } ) => {
  //   const elapsed = clock.getElapsedTime;
  //   rotateB.current.rotation.x = elapsed/5;
  // })
...
useGLTF.preload('/swagJobAnim.glb')
